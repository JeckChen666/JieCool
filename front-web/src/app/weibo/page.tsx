...
export default function WeiboListPage() {
    ...
    const onPreviewImage = async (fileId: number) => {
        try {
            ...
            const url = BASE_URL + (info.download_url || fileApi.getDownloadUrl(uuid));
                                    ...
                                    <Card key={`${it.id}-${idx}`} bordered hoverable>
                                        <Space direction="vertical" size={10} style={{width: '100%'}}>
                                            ...
                                            {imgAssets.length > 0 && (
                                                ...
                                                >
                                                    {imgAssets.map((a, idx) => (
                                                        <div key={`${a.fileId}-${idx}`} style={{flex: '0 0 auto'}}
                                                             onClick={() => onPreviewImage(a.fileId)}>
                                                            <FileThumbnail fileId={a.fileId} size={80}
                                                                           clickable={false}/>
                                                        </div>
                                                    ))}
                                                </div>
