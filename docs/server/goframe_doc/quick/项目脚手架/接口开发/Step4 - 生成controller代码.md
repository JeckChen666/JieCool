...
![goframe api interface controller](scaffold-controller-generated-code.png)
...
用于具体的`api`接口实现的代码文件。默认情况下，会按照一个`api`接口一个源码文件的形式生成代码。当然，也可以控制按照`api`文件定义的接口聚合生成到对应的一个源码文件中。具体的命令介绍及配置请参考章节 [接口规范-gen ctrl](../../../goframe_doc/docs/开发工具/代码生成-gen/接口规范-gen%20ctrl.md)。
