...
import { getDetail, updatePost, AssetInput } from "@/lib/weibo";
import { fileApi } from "@/lib/file-api";
import FileThumbnail from "@/components/FileThumbnail";
import { Typography, Space, Button, Spin, Alert, Form, Input, Select, Grid, Message, Divider, Tag, Card, Modal } from "@arco-design/web-react";
    ...
    getDetail(id)
      .then((res) => {
        ...
        const initAssets: AssetInput[] = Array.isArray(res.assets)
          ? res.assets.map((a) => ({ fileId: a.fileId, kind: (a.kind as any) === 'attachment' ? 'attachment' : 'image' }))
          : [];
            ...
            <Card bordered>
              <Space direction="vertical" style={{ width: '100%' }}>
                <Form.Item label="选择图片上传（可多选）">
                  <input id="editImageInput" type="file" accept="image/*" multiple onChange={onSelectFiles} style={{ display: 'none' }} />
                  <Button type="primary" onClick={() => (document.getElementById('editImageInput') as HTMLInputElement | null)?.click()}>选择文件</Button>
                </Form.Item>
                ...
                .map(({ a, idx }) => (
                  ...
                  >
                    ...
                    <div
                      ...
                      title="拖拽排序"
                      style={{ position: 'absolute', left: 8, top: 8, cursor: 'grab', fontSize: 12, color: '#999' }}
                    >
                      ⇅
                    </div>
                    {/* 操作区 */}
                    <div style={{ position: 'absolute', right: 8, top: 8, display: 'flex', gap: 6 }}>
                      ...
                      <Button size="mini" status="danger" onClick={() => removeAsset(idx)}>删除</Button>
                    </div>
                    ...
                    <Space direction="vertical" size={4} style={{ marginTop: 8 }}>
                      ...
                      <Typography.Text type="secondary">顺序: {a.sortOrder ?? idx}</Typography.Text>
                    </Space>
                  </Card>
                ))}
            </div>
