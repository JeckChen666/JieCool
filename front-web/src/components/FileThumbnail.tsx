...
export default function FileThumbnail({ fileId, size = 100, clickable = true }: Props) {
  ...
  if (error || !thumbUrl) {
    return <div style={{ width: size, height: size, display: "flex", alignItems: "center", justifyContent: "center", background: "#fff1f0", color: "#cf1322", border: "1px solid #ffccc7", borderRadius: 8, fontSize: 12 }}>缩略图加载失败</div>;
  }
  // 统一将相对路径转换为后端绝对地址，避免 Next.js /api 路由重写不生效导致 404
  // 使用前端源地址，确保经过 Next.js rewrites，再由前端代理到后端
  const frontOrigin = typeof window !== "undefined" ? window.location.origin : "http://localhost:3000";
  const rawThumb = thumbUrl.startsWith("http") ? thumbUrl : `${frontOrigin}${thumbUrl}`;
  const sep = rawThumb.includes("?") ? "&" : "?";
  const thumbSrc = `${rawThumb}${sep}width=${size}&height=${size}`;
  const downloadHref = downloadUrl ? (downloadUrl.startsWith("http") ? downloadUrl : `${frontOrigin}${downloadUrl}`) : undefined;
  const img = (
    <img
      src={thumbSrc}
      alt="thumbnail"
